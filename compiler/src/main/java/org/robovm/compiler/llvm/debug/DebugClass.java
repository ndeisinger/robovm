...
public class DebugClass {
    ...
    private ContextDebugStatement context; // Context of class
                                           // (filename/directory)
    ...
    public DebugClass(String className, String fileName, String directory) {
        ...
        subprograms = new SubprogramListDebugStatement(
                DebugManager.getRefInteger(), -1);
        context = new ContextDebugStatement(DebugManager.getRefInteger(), -1,
                fileName, directory);
        nullStmt = new NullDebugStatement(DebugManager.getRefInteger(), -1);
        unit = new CompileUnitDebugStatement(DebugManager.getRefInteger(), -1,
                context.getRef(), subprograms.getRef(), nullStmt.getRef());
        nullType = new TypeDebugStatement(DebugManager.getRefInteger(), -1,
                nullStmt.getRef());
        file = new FileDebugStatement(DebugManager.getRefInteger(), -1,
                context.getRef());
    }
    ...
    public String debugBlocks() {
        ...
        // Note that it doesn't matter what order we print out function/line
        // debug info as long as it's all there
    ...
    /**
     * For use on individual class compilations. Returns debug information with
     * a compile unit list defined containing just this class.
     */
